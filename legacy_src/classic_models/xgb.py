...
def main(dataset, feature):
    ...
    if dataset == 'german':
        labels = np.squeeze((dataset_orig.labels == 1.0).astype(int))
    else:
        labels = np.squeeze(dataset_orig.labels)

    sss = StratifiedShuffleSplit(n_splits=5, test_size=0.3)
    sss_splits = []
    for train_index, test_index in sss.split(dataset_orig.features, labels):
    ...
    for i, (Xtrain, Xtest, Ytrain, Ytest) in tqdm(enumerate(sss_splits)):
        ...
        with wandb.init(**wandb_init) as run:
            ...
            Ytrain_pred = bst.predict(Xtrain)
            Ytest_pred = bst.predict(Xtest)
